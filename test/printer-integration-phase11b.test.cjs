...
test("271 destroy after initialize destroys components", async () => { const core = coreModules(), integration = createPrinterIntegration({ importer: core.importer }); await integration.initialize(); assert.equal(integration.destroy(), true); await integration.destroyAsync(); assert.ok(core.destroyed() >= 2); });
