...
test("349 USB permission stays binding-only while profile saves apply and reload", () => { assert.match(posSource, /requestUsbDeviceBinding/); assert.match(posSource, /listAuthorizedUsbBindings/); assert.match(posSource, /savePrinterProfileAndApply/); assert.match(posSource, /applySavedPrinterConfiguration/); assert.doesNotMatch(posSource, /PrinterCenter\.(?:connectUsbPrinter|disconnectUsbPrinter)/); });
